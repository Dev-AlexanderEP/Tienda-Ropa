...
const CategoryFlyoutContent = ({ title, moda,basicos,accesorios,  images = [], buttonLabel = "Ver Todo", buttonHref = "#", idvertodo }) => (
    ...
    <div className="flex">
      <div className="w-[35%] h-full max-lg:w-[70%]">
        ...
        <div className="w-full grid grid-cols-2 text-gray-600 gap-y-1 mt-2">
          {moda.map(item => (
            <Typography as="a" href={item.href} key={item.label} id={item.label} className="col-span-1">
              {item.label}
            </Typography>
          ))}
        </div>
