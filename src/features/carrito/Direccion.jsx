...
const Direccion = ({ datos, setDatos, onContinuar }) => {
  ...
  useEffect(() => {
    if (datos.departamento) {
      const depObj = provinciasData.find((dep) => dep.departamento_id === datos.departamento);
      setProvincias(depObj ? depObj.provincias : []);
      if (!depObj?.provincias.some((p) => p.id === datos.provincia)) {
        setDatos((prev) => ({ ...prev, provincia: "", distrito: "" }));
        setDistritos([]);
      }
    } else {
      setProvincias([]);
  ...
  useEffect(() => {
    if (datos.provincia) {
      ...
      for (const grupo of distritosData) {
        const provObj = grupo.find((prov) => prov.provincia_id === datos.provincia);
        if (provObj) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const result = DireccionSchema.safeParse(datos);
    if (!result.success) {
    ...
    <div className="w-[600px] border border-gray-200 p-5 max-lg:w-auto">
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div>
          ...
          >
            ...
            {departamentos.map((dep) => (
              <option key={dep.id} value={dep.id} className="font-Poppins">{dep.nombre}</option>
            ))}
          </select>
          {errores.departamento && <p className="text-red-500 text-sm">{errores.departamento[0]}</p>}
        </div>
        <div>
        ...
        <div>
          ...
          {errores.distrito && <p className="text-red-500 text-sm">{errores.distrito[0]}</p>}
        </div>
        <div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Typography as="label" htmlFor="guardarData2" className="cursor-pointer text-foreground font-Poppins">
